...
function FestivalGoerDashboard() {
  const { connected, rounds, offers, log, openRound } = useMarketSocket("festival_goer");
  ...
  return (
    <div className="mx-auto max-w-3xl p-6">
      ...
      <p className="mb-6 text-sm" style={{ color: "var(--muted)" }}>
        ...
        threshold, it opens a bidding round to the WiFi and Power provider agents.
      </p>

      <div className="mb-6 grid grid-cols-1 gap-4 rounded-xl border p-4 sm:grid-cols-3" style={{ borderColor: "var(--hairline)" }}>
      ...
      <button
        type="button"
        disabled={!belowThreshold || hasOpenRound}
        onClick={() => openRound(batteryPct, wifiSignalPct, capUsd)}
        className="mb-6 rounded-lg px-5 py-2 text-sm font-semibold text-white disabled:opacity-40"
        style={{ background: "var(--accent)" }}
      >
        {hasOpenRound ? "Round already open" : belowThreshold ? "Request offers now" : "Above threshold — lower battery or signal to trigger"}
      </button>
